...
function resolveStoredFile(storagePath){const raw=String(storagePath||"");const normalized=raw.replace(/\\/g,"/");if(!normalized||normalized.includes("..")||path.posix.isAbsolute(normalized)||path.win32.isAbsolute(raw)||/^[A-Za-z]:/.test(normalized))throw new Error("invalid report storage path");const resolved=path.resolve(STORAGE_ROOT,normalized);if(!resolved.startsWith(`${STORAGE_ROOT}${path.sep}`))throw new Error("report path escapes root");return resolved;}
