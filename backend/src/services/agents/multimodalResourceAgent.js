...
async function generateStructuredResource(context) {
  ...
  try{
    const prompt=["Return strict JSON only. Never use HTML, URLs, file paths, or IDs outside allowed lists.",JSON.stringify(minimalModelContext(context)),"Required schema follows the LearnMate mind_map or pptx contract."].join("\n");
...
function buildFallback(c){return validateResourceEnvelope(c.resourceType==="mind_map"?mindMap(c):pptx(c),c);}
function base(c,content){return{resourceType:c.resourceType,title:`${c.knowledgePoint}个性化${c.resourceType==="mind_map"?"思维导图":"学习课件"}`,subject:c.subject,knowledgePoint:c.knowledgePoint,learningObjectives:c.stageGoals.slice(0,5),targetLearnerSummary:`当前掌握度 ${c.mastery}，偏好${c.explanationPreference}。`,estimatedMinutes:c.estimatedMinutes,generationRationale:[`路径阶段：${c.stageTitle}`,`当前掌握度：${c.mastery}`,c.errorPatterns[0]?`主要错误模式：${c.errorPatterns[0].errorType}`:"暂无高频错误模式"],content};}
function mindMap(c){const kp=c.knowledgePoint;const err=c.errorPatterns[0]?.errorType||"易错点复盘";return base(c,{root:{id:"root",label:kp,description:`${c.stageTitle}的核心知识结构`,nodeType:"root",importance:"high",children:[
...
function pptx(c){const kp=c.knowledgePoint;const q=c.allowedQuestionIds.slice(0,3);const err=c.errorPatterns[0]?.errorType||"概念边界混淆";return base(c,{theme:{name:"academic-light",primaryTone:"blue",density:"medium"},slides:[
