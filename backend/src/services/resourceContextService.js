...
async function buildResourceContext({ studentId, resourceType, subject, knowledgePoint, stageKey, pathVersion }) {
  ...
  if(!["mind_map","pptx"].includes(resourceType)) throw apiError("resourceType must be mind_map or pptx",400);
  ...
  const [[profile],mastery,errors,questions,codes] = await Promise.all([
    ...
    pool.query(`SELECT question_id,stem,analysis,difficulty FROM question_bank WHERE question_id IN (${placeholders(stage.questionIds)})`,stage.questionIds||[]),
    pool.query(`SELECT exercise_id,title,description,language FROM code_exercises WHERE exercise_id IN (${placeholders(stage.codeExerciseIds)})`,stage.codeExerciseIds||[])
  ]);
  const masteryRow=mastery[0][0]||{}; const estimatedMinutes=Math.max(10,Math.min(120,Number(stage.durationMinutes)||30));
  return {
    studentId:id,resourceType,subject,knowledgePoint,pathId:`student-${id}`,pathVersion:Number(pathRow.version),stageKey,
    ...
    questionEvidence:questions[0].map((row)=>({questionId:row.question_id,stem:row.stem,analysis:row.analysis,difficulty:row.difficulty})),
    codeEvidence:codes[0].map((row)=>({exerciseId:row.exercise_id,title:row.title,description:row.description,language:row.language}))
  };
}
