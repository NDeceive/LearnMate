...
function fail(res,error){const status=error.statusCode||500,requestId=res.req?.requestId;console.error(JSON.stringify({level:"error",requestId,status,area:"teacher",code:error.code||"ERROR",message:status<500?String(error.message).slice(0,160):"teacher operation failed"}));return res.status(status).json({error:status<500?error.message:"教师端操作失败",code:status===403?"FORBIDDEN":undefined,requestId});}
